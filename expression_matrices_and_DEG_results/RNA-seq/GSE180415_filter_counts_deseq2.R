setwd("/Lung_data/Fibroblast/RNA-seq/GSE180415/expression_data/expression_matrices")


raw_matrix <- read.csv("raw_counts_GSE180415.csv", sep="\t")
...
metadata <- read.xlsx("/Lung_data/Fibroblast/RNA-seq/GSE180415/phenodata/GSE180415_curated.xlsx", sheetIndex = 1)
...
write.table(filtered_data, file = "Filtered_expression_matrix_ensembl_GSE180415.csv", sep = "\t", col.names =TRUE)
...
write.table(aggr_table, file = "Filtered_expression_matrix_symbol_GSE180415.csv", sep = "\t", col.names =TRUE)
...
ggsave(file="GSE180415_pca_before_normalization.svg", width=10, height=8)

### Differential expression analysis symbol ###

setwd("/Lung_data/Fibroblast/RNA-seq/GSE180415/expression_data/expression_matrices")
expr_mat<-read.table("Filtered_expression_matrix_symbol_GSE180415.csv", sep = "\t")
metadata <- read.xlsx("/Lung_data/Fibroblast/RNA-seq/GSE180415/phenodata/GSE180415_curated.xlsx", sheetIndex = 1)
...
conditions<-metadata$disease #no other relevant conditions
...
write.table(total_norm_counts, file = "Normalized_expression_matrix_symbol_GSE180415.txt", quote = FALSE, sep = "\t", row.names = TRUE, col=NA)
...
ggsave(file="GSE180415_pca_after_normalization.svg", width=10, height=8)

####################################################################################

setwd("/Lung_data/Fibroblast/RNA-seq/GSE180415/expression_data/DEG_results")

res1 <- DESeq2::results(dds, contrast = c("condition", "IPF", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_IPF_vs_healthy_unfiltered_symbol_GSE180415.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_IPF_vs_healthy_filtered_symbol_GSE180415.txt", row.names = TRUE, sep = "\t", quote = FALSE)

### Differential expression analysis ensembl###

setwd("/Lung_data/Fibroblast/RNA-seq/GSE180415/expression_data/expression_matrices")
expr_mat<-read.table("Filtered_expression_matrix_ensembl_GSE180415.csv", sep = "\t")
metadata <- read.xlsx("/Lung_data/Fibroblast/RNA-seq/GSE180415/phenodata/GSE180415_curated.xlsx", sheetIndex = 1)
...
dds <- DESeq2::DESeq(ddsMat)
total_norm_counts <- DESeq2::counts(dds, normalized=TRUE)
write.table(total_norm_counts, file = "Normalized_expression_matrix_ensembl_GSE180415.txt", quote = FALSE, sep = "\t", row.names = TRUE, col=NA)

setwd("/Lung_data/Fibroblast/RNA-seq/GSE180415/expression_data/DEG_results")
res1 <- DESeq2::results(dds, contrast = c("condition", "IPF", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_IPF_vs_healthy_unfiltered_ensembl_GSE180415.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_IPF_vs_healthy_filtered_ensembl_GSE180415.txt", row.names = TRUE, sep = "\t", quote = FALSE)
