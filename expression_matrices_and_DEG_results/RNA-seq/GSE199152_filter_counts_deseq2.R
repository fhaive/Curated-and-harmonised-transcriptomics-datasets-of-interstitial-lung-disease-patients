setwd("/Lung_data/Biopsy/RNA-seq/GSE199152/expression_data/expression_matrices")


raw_matrix <- read.csv("GSE199152_rawcounts.csv", sep="\t")
...
metadata <- read.xlsx("/Lung_data/Biopsy/RNA-seq/GSE199152/phenodata/GSE199152_curated.xlsx", sheetIndex = 1)

conditions<-(metadata$disease)

###metadata reversed to samples, lets reorder the raw_matrix
...
write.table(filtered_data, file = "Filtered_expression_matrix_ensembl_GSE199152.csv", sep = "\t", col.names =TRUE)
...
write.table(aggr_table, file = "Filtered_expression_matrix_symbol_GSE199152.csv", sep = "\t", col.names =TRUE)
...
ggsave(file="GSE199152_pca_before_normalization.svg", width=10, height=8)


### Differential expression analysis ###

setwd("/Lung_data/Biopsy/RNA-seq/GSE199152/expression_data/expression_matrices")
expr_mat<-read.table("Filtered_expression_matrix_symbol_GSE199152.csv", sep = "\t")
metadata <- read.xlsx("/Lung_data/Biopsy/RNA-seq/GSE199152/phenodata/GSE199152_curated.xlsx", sheetIndex = 1)
...
#No other relevant variables in metadata
###############################################################
...
ggsave(file="GSE199152_pca_after_normalization.svg", width=10, height=8)


#################################################################

setwd("/Lung_data/Biopsy/RNA-seq/GSE199152/expression_data/expression_matrices")
write.table(total_norm_counts, file = "Normalized_expression_matrix_symbol_GSE199152.txt", quote = FALSE, sep = "\t", row.names = TRUE, col=NA)


setwd("/Lung_data/Biopsy/RNA-seq/GSE199152/expression_data/DEG_results")
res1 <- DESeq2::results(dds, contrast = c("condition", "UIP", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_UIP_vs_healthy_unfiltered_symbol_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_UIP_vs_healthy_filtered_symbol_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)

res1 <- DESeq2::results(dds, contrast = c("condition", "Rheumatoid_Arthritis_Associated_UIP", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_RA-UIP_vs_healthy_unfiltered_symbol_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_RA-UIP_vs_healthy_filtered_symbol_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)
...
######################Ensembl
setwd("/Lung_data/Biopsy/RNA-seq/GSE199152/expression_data/expression_matrices")
expr_mat<-read.table("Filtered_expression_matrix_ensembl_GSE199152.csv", sep = "\t")
metadata <- read.xlsx("/Lung_data/Biopsy/RNA-seq/GSE199152/phenodata/GSE199152_curated.xlsx", sheetIndex = 1)
...
total_norm_counts <- DESeq2::counts(dds, normalized=TRUE)
setwd("/Lung_data/Biopsy/RNA-seq/GSE199152/expression_data/expression_matrices")
write.table(total_norm_counts, file = "Normalized_expression_matrix_ensembl_GSE199152.txt", quote = FALSE, sep = "\t", row.names = TRUE, col=NA)


setwd("/Lung_data/Biopsy/RNA-seq/GSE199152/expression_data/DEG_results")
res1 <- DESeq2::results(dds, contrast = c("condition", "UIP", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_UIP_vs_healthy_unfiltered_ensembl_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_UIP_vs_healthy_filtered_ensembl_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)

res1 <- DESeq2::results(dds, contrast = c("condition", "Rheumatoid_Arthritis_Associated_UIP", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_RA-UIP_vs_healthy_unfiltered_ensembl_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_RA-UIP_vs_healthy_filtered_ensembl_GSE199152.txt", row.names = TRUE, sep = "\t", quote = FALSE)
