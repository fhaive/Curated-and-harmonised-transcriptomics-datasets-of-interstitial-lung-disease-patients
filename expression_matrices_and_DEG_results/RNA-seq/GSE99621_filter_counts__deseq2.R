setwd("/Lung_data/Biopsy/RNA-seq/GSE99621/expression_data/expression_matrices")

raw_matrix <- read.csv("GSE99621_rawcounts.csv", sep="\t")
...
metadata <- read.xlsx("/Lung_data/Biopsy/RNA-seq/GSE99621/phenodata/GSE99621_curated.xlsx", sheetIndex = 1)

conditions<-(metadata$disease)

###metadata and data samples in the same order
...
write.table(filtered_data, file = "Filtered_expression_matrix_ensembl_GSE99621.csv", sep = "\t", col.names =TRUE)
...
write.table(aggr_table, file = "Filtered_expression_matrix_symbol_GSE99621.csv", sep = "\t", col.names =TRUE)
...
ggsave(file="GSE99621_pca_before_normalization.svg", width=10, height=8)

### Differential expression analysis ###

setwd("/Lung_data/Biopsy/RNA-seq/GSE99621/expression_data/expression_matrices")
expr_mat<-read.table("Filtered_expression_matrix_symbol_GSE99621.csv", sep = "\t")
...
conditions<-metadata$disease
source<-metadata$sample_source #linear combination of condition
appearance<-metadata$microscopic_appearance

colData <- data.frame(condition=as.vector(conditions), source=as.vector(source), appearance=as.vector(appearance))
rownames(colData) <- colnames(expr_mat)


ddsMat <- DESeq2::DESeqDataSetFromMatrix(countData = table_1,
                                         colData = colData,
                                         design = ~appearance+condition)

dds <- DESeq2::DESeq(ddsMat)
total_norm_counts <- DESeq2::counts(dds, normalized=TRUE)
write.table(total_norm_counts, file = "Normalized_expression_matrix_symbol_GSE99621.txt", quote = FALSE, sep = "\t", row.names = TRUE, col=NA)
...
ggsave(file="GSE99621_pca_after_normalization.svg", width=10, height=8)
######################################################################
setwd("/Lung_data/Biopsy/RNA-seq/GSE99621/expression_data/DEG_results")
res1 <- DESeq2::results(dds, contrast = c("condition", "IPF", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_IPF_vs_healthy_unfiltered_symbol_GSE99621.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_IPF_vs_healthy_filtered_symbol_GSE99621.txt", row.names = TRUE, sep = "\t", quote = FALSE)

### Differential expression analysis ###
##############3ENSEMBL#####################

setwd("/Lung_data/Biopsy/RNA-seq/GSE99621/expression_data/expression_matrices")
expr_mat<-read.table("Filtered_expression_matrix_ensembl_GSE99621.csv", sep = "\t")
...
dds <- DESeq2::DESeq(ddsMat)
total_norm_counts <- DESeq2::counts(dds, normalized=TRUE)
write.table(total_norm_counts, file = "Normalized_expression_matrix_ensembl_GSE99621.txt", quote = FALSE, sep = "\t", row.names = TRUE, col=NA)

setwd("/Lung_data/Biopsy/RNA-seq/GSE99621/expression_data/DEG_results")
res1 <- DESeq2::results(dds, contrast = c("condition", "IPF", "healthy"), pAdjustMethod = "fdr", independentFiltering = FALSE)
write.table(res1, file = "DEG_results_DESeq2_IPF_vs_healthy_unfiltered_ensembl_GSE99621.txt", row.names = TRUE, sep = "\t", quote = FALSE)
res1_adj <- res1[which(res1$padj<=0.01 & abs(res1$log2FoldChange)>=0.58),]
print(dim(res1_adj))
write.table(res1_adj, file = "DEG_results_DESeq2_IPF_vs_healthy_filtered_ensembl_GSE99621.txt", row.names = TRUE, sep = "\t", quote = FALSE)
